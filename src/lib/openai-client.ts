...
const openai = new OpenAI({
  apiKey: process.env.NEXT_PUBLIC_OPENAI_API_KEY || 'placeholder',
...
`
    ...
    if (error.message === 'API_KEY_NOT_CONFIGURED') {
      throw new Error('⚠️ API Key da OpenAI não configurada.\n\nPara usar a análise por IA:\n1. Clique no banner laranja acima\n2. Adicione sua chave NEXT_PUBLIC_OPENAI_API_KEY\n3. Obtenha sua chave em: https://platform.openai.com/api-keys')
    }
    throw error
  }
}
...
export async function transcribeAudio(audioBlob: Blob): Promise<string> {
  try {
    ...
    if (error.message === 'API_KEY_NOT_CONFIGURED') {
      throw new Error('⚠️ API Key da OpenAI não configurada.\n\nPara usar a transcrição de áudio:\n1. Clique no banner laranja acima\n2. Adicione sua chave NEXT_PUBLIC_OPENAI_API_KEY\n3. Obtenha sua chave em: https://platform.openai.com/api-keys')
    }
    throw error
  }
}
